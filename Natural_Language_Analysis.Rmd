...
df4 %>% datatable(.,caption="Table 3.4.1 : Partial Matching Teams' Name.")
```

  Below table simply display few matched teams' name which are not accurate.

```{r matching-04B, echo=FALSE, results='asis'}
df %>% filter((teamID %in% teamID[grep('Women|U[0-9]{2}',teamID)])|(spboID %in% spboID[grep('Women|U[0-9]{2}',spboID)]), Match=='Partial') %>% head %>% kable
```
