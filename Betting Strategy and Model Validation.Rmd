...
## Annual summary
res <- ldply(split(mbase$datasets, mbase$datasets$Sess), function(x) data.frame(Stakes=sum(x$Stakes), S.median=median(x$Stakes), S.mean=mean(x$Stakes), S.sd=sd(x$Stakes), Count=length(x$PL), Return=sum(x$PL,x$Stakes), PL=sum(x$PL), PL.percent=sum(x$PL)/sum(x$Stakes)))
names(res)[1] <- 'Sess'
res %>% kable(.,caption='Table 3.1.1 : Annual Summary of Staking Data.')
```
...
ahBets <- data.frame(ddply(mbase2, ~HCap+AHOU+Picked,
                     ...
                     ddply(mbase2, ~HCap+AHOU+Picked, .drop=TRUE, colwise(.fun=length, .cols=~round(Stakes,2)))[-c(1:3)])
...
line.gvis <- gvisLineChart(xvar="HCap", yvar=c("value"), data=melted, options=gvis.options)
plot(line.gvis)
...
line.gvis <- gvisLineChart(xvar="pHKRange", yvar=c("value"), data=melted, options=gvis.options)
plot(line.gvis)
...
ipBets1$R.percent <- round(ipBets1$Return/ipBets1$Stakes,4)
ipBets1$PL.percent <- round(ipBets1$PL/ipBets1$Stakes,4)
...
line.gvis <- gvisLineChart(xvar="ipRange", yvar=c("value"), data=melted, options=gvis.options)
plot(line.gvis)
...
ipBets2 <- data.frame(ddply(mbase2, ~InPlay2+HCap+AHOU+CurScore+ipHCap+ipRange+Picked+Picked2,
                      ...
                      ddply(mbase2, ~InPlay2+HCap+AHOU+CurScore+ipHCap+ipRange+Picked+Picked2,
                            .drop=TRUE, colwise(.fun=length, .cols=~round(Stakes,2)))[-c(1:8)])
...
ipBets2$R.percent <- round(ipBets2$Return/ipBets2$Stakes,2)
ipBets2$PL.percent <- round(ipBets2$PL/ipBets2$Stakes,2)
...
mResult$S.prop <- round(mResult$Stakes/sum(mResult$Stakes),4)
mResult$R.prop <- round(mResult$Return/sum(mResult$Return),4)
mResult$prop <- round(mResult$n/sum(mResult$n),4)
